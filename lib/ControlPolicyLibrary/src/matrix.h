...
namespace cpl{
    ...
    class Matrix{
        ...
        // Retrieves the first element of the matrix
        // Most useful for getting the value of 1x1 matrices
        operator Scalar() {return data[0][0];}
        ...
        Matrix<n, m>
        elementwise_multiplication(const Matrix<n,m> &other) const
        {
            Matrix<n, m> M;
            for(Size i = 0; i < n; i++)
            for(Size j = 0; j < m; j++)
